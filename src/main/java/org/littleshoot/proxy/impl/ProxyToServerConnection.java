...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
	...
	static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
			RouteId routeId, HttpFilters initialFilters, HttpRequest initialHttpRequest,
		...
		if (chainedProxyManager != null) {
			chainedProxyManager.lookupChainedProxies(initialHttpRequest, chainedProxies,
					clientConnection.getClientDetails());
			if (chainedProxies.size() == 0) {
				// ChainedProxyManager returned no proxies, can't connect
				return null;
			}
		}
		return new ProxyToServerConnection(proxyServer, clientConnection, routeId, chainedProxies.poll(),
				chainedProxies, initialFilters, globalTrafficShapingHandler);
	}

	private ProxyToServerConnection(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
			RouteId routeId, ChainedProxy chainedProxy, Queue<ChainedProxy> availableChainedProxies,
			HttpFilters initialFilters, GlobalTrafficShapingHandler globalTrafficShapingHandler)
			throws UnknownHostException {
	...
	private void initializeConnectionFlow() {
		...
		if (ProxyUtils.isCONNECT(initialRequest)) {
			...
			if (isMitmEnabled) {
				...
				HostAndPort parsedHostAndPort = HostAndPort.fromString(routeId.getHostAndPort());
	 ...
	 */
	private ConnectionFlowStep SOCKS4CONNECTWithChainedProxy = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {

		@Override
		protected Future<?> execute() {
			InetSocketAddress destinationAddress;
			try {
				destinationAddress = addressFor(routeId.getHostAndPort(), proxyServer);
			} catch (UnknownHostException e) {
				return channel.newFailedFuture(e);
			}

			DefaultSocks4CommandRequest connectRequest = new DefaultSocks4CommandRequest(Socks4CommandType.CONNECT,
	 ...
	 */
	private ConnectionFlowStep SOCKS5InitialRequest = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {
	...
	/**
	 * Sends SOCKS5 password credentials after {@link #SOCKS5InitialRequest} has
	 * completed.
	 */
	private ConnectionFlowStep SOCKS5SendPasswordCredentials = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {
	 ...
	 */
	private ConnectionFlowStep SOCKS5CONNECTRequestWithChainedProxy = new ConnectionFlowStep(this,
			AWAITING_CONNECT_OK) {

		@Override
		protected Future<?> execute() {
			InetSocketAddress destinationAddress = unresolvedAddressFor(routeId.getHostAndPort());
			DefaultSocks5CommandRequest connectRequest = new DefaultSocks5CommandRequest(Socks5CommandType.CONNECT,
					Socks5AddressType.DOMAIN, destinationAddress.getHostString(), destinationAddress.getPort());
	...
	private void setupConnectionParameters() throws UnknownHostException {
		if (chainedProxy != null && chainedProxy != ChainedProxyAdapter.FALLBACK_TO_DIRECT_CONNECTION) {
			...
			this.remoteAddress = this.currentFilters.proxyToServerResolutionStarted(routeId.getHostAndPort());
			...
			try {
				if (this.remoteAddress == null) {
					hostAndPort = routeId.getHostAndPort();
					this.remoteAddress = addressFor(routeId.getHostAndPort(), proxyServer);
			...
			this.currentFilters.proxyToServerResolutionSucceeded(routeId.getHostAndPort(), this.remoteAddress);
	...
	void connectionSucceeded(boolean shouldForwardInitialRequest) {
		...
		// counted (typically because the HttpObjectAggregator is in
	...
	/**
	 * Similar to {@link #addressFor(String, DefaultHttpProxyServer)} except that it
