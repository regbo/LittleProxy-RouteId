...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
	 ...
	 */
	private static final String LOWERCASE_TRANSFER_ENCODING_HEADER = HttpHeaders.Names.TRANSFER_ENCODING
	 ...
	 */
	private final Map<RouteId, ProxyToServerConnection> serverConnectionsByRouteId = new ConcurrentHashMap<>();
	 ...
	 * chunked data.
	 */
	private volatile RouteId currentProxyToServerRouteId;
	...
	ClientToProxyConnection(final DefaultHttpProxyServer proxyServer, SslEngineSource sslEngineSource,
	...
	private ConnectionState doReadHTTPInitial(HttpRequest httpRequest) {
		...
		RouteId requestRouteId = RouteId.build(proxyServer, clientDetails, httpRequest);
		LOG.debug("Ensuring that hostAndPort are available in {}", httpRequest.uri());
		if (requestRouteId == null || StringUtils.isBlank(requestRouteId.getHostAndPort())) {
		...
		LOG.debug("Finding ProxyToServerConnection for: {}", requestRouteId);
		ProxyToServerConnection currentProxyToServerConnection;
		if (isMitming()) {
			if (!routeIdsEqual(this.currentProxyToServerRouteId, requestRouteId)) {
			...
			currentProxyToServerConnection = this.getCurrentProxyToServerConnection();
		} else if (isTunneling()) {
			currentProxyToServerConnection = this.getCurrentProxyToServerConnection();
		} else {
			currentProxyToServerConnection = this.serverConnectionsByRouteId.get(requestRouteId);
			this.currentProxyToServerRouteId = requestRouteId;
		}
		...
		} else if (currentProxyToServerConnection == null) {
			LOG.debug("Didn't find existing ProxyToServerConnection for: {}", requestRouteId);
			newConnectionRequired = true;
		}

		if (newConnectionRequired) {
			try {
				currentProxyToServerConnection = ProxyToServerConnection.create(proxyServer, this, requestRouteId,
						currentFilters, httpRequest, globalTrafficShapingHandler);
				if (currentProxyToServerConnection == null) {
				...
				serverConnectionsByRouteId.put(requestRouteId, currentProxyToServerConnection);
			...
			LOG.debug("Reusing existing server connection: {}", currentProxyToServerConnection);
		...
		currentProxyToServerConnection.write(httpRequest, currentFilters);
	...
	private boolean routeIdsEqual(RouteId rid1, RouteId rid2) {
		if (rid1 == null && rid2 == null)
			return true;
		if (rid1 == null || rid2 == null)
			return false;
		if (!Objects.equals(rid1.getProxyHostAndPort(), rid2.getProxyHostAndPort()))
			return false;
		Function<RouteId, String> routeIdToServerAndHost = rid -> {
			String hostAndPort = rid.getHostAndPort();
			hostAndPort = hostAndPort == null ? "" : hostAndPort;
			hostAndPort = StringUtils.substringBefore(hostAndPort, ":");
			return hostAndPort;
		};
		boolean equal = Objects.equals(routeIdToServerAndHost.apply(rid1), routeIdToServerAndHost.apply(rid2));
		return equal;
	}

	private ProxyToServerConnection getCurrentProxyToServerConnection() {
		if (this.currentProxyToServerRouteId == null)
			return null;
		return serverConnectionsByRouteId.get(this.currentProxyToServerRouteId);
	}
	...
	protected void readHTTPChunk(HttpContent chunk) {
		...
		this.getCurrentProxyToServerConnection().write(chunk);
	}

	@Override
	protected void readRaw(ByteBuf buf) {
		this.getCurrentProxyToServerConnection().write(buf);
	}
	...
	void timedOut(ProxyToServerConnection serverConnection) {
		if (this.getCurrentProxyToServerConnection() == serverConnection
				&& this.lastReadTime > this.getCurrentProxyToServerConnection().lastReadTime) {
			// the idle timeout fired on the active server connection. send a timeout
			// response to the client.
			LOG.warn("Server timed out: {}", this.getCurrentProxyToServerConnection());
	...
	protected void timedOut() {
		...
		if (this.getCurrentProxyToServerConnection() == null
				|| this.lastReadTime <= this.getCurrentProxyToServerConnection().lastReadTime) {
			super.timedOut();
		}
	}
	...
	protected void disconnected() {
		super.disconnected();
		for (ProxyToServerConnection serverConnection : serverConnectionsByRouteId.values()) {
	...
	private void connectionFailedUnrecoverably(HttpRequest initialRequest, ProxyToServerConnection serverConnection) {
		...
		serverConnection.disconnect();
		this.serverConnectionsByRouteId.remove(serverConnection.getRouteId());
	...
	synchronized protected void serverBecameWriteable(ProxyToServerConnection serverConnection) {
		...
		for (ProxyToServerConnection otherServerConnection : serverConnectionsByRouteId.values()) {
	 ...
	 * 
	 * @return True if the proxy server set to accept a proxy protocol header. False
	 *         otherwise
	 */
	 ...
	 * 
	 * @return True if the proxy server set to send a proxy protocol header. False
	 *         otherwise
	 */
	...
	private boolean isNextHopOriginServer() {
		// If there is no upstream chained proxy, the next hop must be the origin
		// server.
		if (!this.getCurrentProxyToServerConnection().hasUpstreamChainedProxy()) {
			return true;
		}

		/*
		 * Upstream SOCKS proxies are a special case because they do not parse or modify
		 * the HTTP request in any way. If the upstream chained proxy is a SOCKS proxy,
		 * we should treat it as if we are connecting directly to the origin server.
		 */
		switch (this.getCurrentProxyToServerConnection().getChainedProxyType()) {
		case HTTP:
			return false;
		case SOCKS4:
		case SOCKS5:
		...
		default:
			LOG.warn("Assuming upstream chained proxy of unknown type "
					+ this.getCurrentProxyToServerConnection().getChainedProxyType()
	 ...
	 * {@link HttpFilters#proxyToClientResponse(HttpObject)} filter method before
	...
	private boolean respondWithShortCircuitResponse(HttpResponse httpResponse) {
		...
		// "as if" the short-circuit response were proxied
	...
	private FlowContext flowContext() {
		if (this.getCurrentProxyToServerConnection() != null) {
			return new FullFlowContext(this, this.getCurrentProxyToServerConnection());
		} else {
